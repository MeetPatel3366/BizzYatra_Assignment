...
if(isset($_POST['accept_friend'])) {
    ...
    $sql="INSERT INTO `friend`(`user_email`, `friend_email`) VALUES  ('$current_user', '$requsted_email')";
    ...
    {   
        ...
        if($dresult)
        {
        echo "<p class='w-full text-center p-3 bg-slate-900 text-green-500'>Accepted friend request from: $requsted_email</p>";  
        }
    }
} 
...
if(isset($_POST['reject_friend'])) {
    ...
    if($result)
    {
        echo "<p class='w-full text-center p-3 bg-slate-900 text-red-500'>reject friend request from: $requsted_email</p>";
    }
}
?>
<!DOCTYPE html>
...
<?php
    include './requirement/dbconnect.php';
    $current_user=$_SESSION['email'];
    $received_request="SELECT DISTINCT sender FROM `friend_request_receive` WHERE `reciver`='$current_user'";
    ...
    if($num>0)
    {
        ...
        echo "<td class='px-28'>" . $row['sender'] . "</td>";
